document.addEventListener('DOMContentLoaded', _ => {
  ...
  inputs.forEach((input, index) => input.setAttribute('id', panes[index].id))
  labels.forEach((label, index) => label.setAttribute('for', panes[index].id))
  ...
  if (window.location.hash) {
    ...
    if (selected) {
      const label = Array.from(labels).find(label => label.getAttribute('for') === selected.getAttribute('id'))
  ...
  labels.forEach(label => label.addEventListener('click', evt => {
    const pane = Array.from(panes).find(pane => pane.getAttribute('id') === label.getAttribute('for'))
